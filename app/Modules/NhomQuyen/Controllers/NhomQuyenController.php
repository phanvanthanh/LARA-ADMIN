...
class NhomQuyenController extends Controller{
    ...
    public function nhomQuyen(Request $request){
        return view('NhomQuyen::nhom-quyen');
    }
    ...
    public function danhSachNhomQuyen(Request $request){
        if(RequestAjax::ajax()){ // Kiểm tra gửi đường ajax
            $error=''; // Khai báo biến
            $roles = AdminRole::all()->toArray(); // điều kiện nhóm quyền còn hoạt động
    ...
    public function nhomQuyenSingle(Request $request){
        if(RequestAjax::ajax()){ // Kiểm tra gửi đường ajax
            ...
            $view=view('NhomQuyen::nhom-quyen-single', compact('data','error'))->render(); // Trả dữ liệu ra view trước     
